...
//  IR-Raytracing
//
//  Created by Max PRUDHOMME on 24/10/2025.
//
...
class Scene_Three {
    ...
    func trace(ray: Ray) -> SIMD4<UInt8> {
        ...
        if let hit = closest {
            let shadowRay = Ray(
                direction: normalize(lights[0].origin - hit.point),
                origin: hit.point + hit.normal * 0.001
            )
            
            let lit = light(hit: hit)
            
            if shadow(ray: shadowRay) {
                let ambient: Float = 0.1
                return SIMD4<UInt8>(
                    UInt8(Float(lit.x) * ambient),
                    UInt8(Float(lit.y) * ambient),
                    UInt8(Float(lit.z) * ambient),
                    255
                )
            } else {
                return lit
    ...
    func light(hit: Hit) -> SIMD4<UInt8> {
        let light = lights[0]
    ...
    func shadow(ray: Ray) -> Bool {
        let lightDistance = length(lights[0].origin - ray.origin)
        
        for sphere in spheres {
            if let hit = sphere.hit(ray: ray) {
                if hit.l < lightDistance {
                    return true
                }
            }
        }
